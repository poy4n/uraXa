					...
					.then((postRecord) => {
						if (postRecord.rows.length > 0) {
							...
							Promise.all(
								...
								.then((postAndImages) => res.json({ posts: postAndImages }))
								.catch((err) => res.status(500).send({ error: err.message }));
						} else {
...
router.post('/post', parser.array('image'), (req, res) => {
	...
	auth
		...
		.then((userRecord) => {
			console.log(userRecord);
			if (!_.isEmpty(userRecord)) {
				Post.createPost(title, text, location, userRecord.id)
					.then((postRecord) => {
						const post = postRecord.rows[0];
						...
						Promise.all(
							...
							.then((postAndImages) => res.status(201).json({ posts: postAndImages }))
							.catch((err) => res.status(500).send({ error: err.message }));
					})
					.catch((err) => res.status(500).send({ error: err.message }));
			} else {
		...
		.then((userRecord) => {
			if (!_.isEmpty(userRecord)) {
				Post.deletePostsByUserId(userRecord.id)
					.then((deletedPostsRecords) => res.json({ post: deletedPostsRecords.rows }))
					.catch((err) => res.status(500).send({ error: err.message }));
			} else {
				res.status(401).send({ error: 'Unauthenticated user. Please login.' });
			}
